...
def pg_allow_remote_access_for_EVERYONE():
    ...
    sudo("""su postgres -c 'echo "# Generated by mup.py">> /etc/postgresql/9.3/main/postgresql.conf'""")
    sudo("""su postgres -c 'echo "listen_addresses = '"'"'*'"'"' ">> /etc/postgresql/9.3/main/postgresql.conf'""")

    sudo("""su postgres -c 'echo "# Generated by mup.py" >> /etc/postgresql/9.3/main/pg_hba.conf'""")
    sudo("""su postgres -c 'echo "host     all             all             0.0.0.0/0               md5" >> /etc/postgresql/9.3/main/pg_hba.conf'""")
...
def sys_create_openerp_user():
    ...
    if env.adm_user_is_sudoer:
        ...
        sudo('sh -c "echo "%s      ALL = /etc/init.d/openerp-server,/etc/init.d/gunicorn-openerp" > /etc/sudoers.d/muppy' % env.adm_user)
